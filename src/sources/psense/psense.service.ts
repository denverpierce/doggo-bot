...
export const PSenseService = (token: string) => {
  ...
  // TODO: use the below object
  const pSenseDefaultArgs = {
    agg: 'HL',
    level: 2
  }
  ...
  return {
    getLatestPollen(location: AmbLocation) {
      const url = `${PSENSE_API}/api/point?lat=${location.lat}&lon=${location.lng}&units=metric&starting=${start}&key=${token}&agg=HL&level=2`
